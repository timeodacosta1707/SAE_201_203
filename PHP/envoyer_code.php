<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (mail($to, $subject, $message, $headers)) {
            header("Location: " . (($_GET['from'] === "index") ? "verification_code.php?from=index" : "verification_code.php?from=profile"));
            exit();
        } else {
            header("Location: mdp_oublie.php?erreur=2" . (($_GET['from'] === "index") ? "&from=index" : "&from=profile"));
            exit();
        }
    } 
?>
