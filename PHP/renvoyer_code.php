<?php
    ...
    if (mail($email, $subject, $message, $headers)) {
        header("Location: verification_code.php?renvoyer=true"  . (($_GET['from'] === "index") ? "&from=index" : "&from=profile"));
        exit();
    } else {
        header("Location: verification_code.php?erreur=1"  . (($_GET['from'] === "index") ? "&from=index" : "&from=profile"));
        exit();
    }
?>
